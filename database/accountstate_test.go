...
func createTestAccountStateData(db bank.Database) AccountStateTestData {
	...
	currTest1, _ := AddOrUpdateCurrency(db, model.NewCurrency("TBTC", "", 1, FlagCurencyAvailable, 1, 8))
	currTest2, _ := AddOrUpdateCurrency(db, model.NewCurrency("EUR", "", 0, FlagCurencyAvailable, 0, 2))
	_, _ = AddOrUpdateCurrency(db, model.NewCurrency("CHF", "", 0, FlagCurencyAvailable, 0, 2))
